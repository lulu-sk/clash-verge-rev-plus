...
function NormalProxyGroups(props: { mode: string }) {
  ...
  const speedTestGroup = useMemo(() => {
    if (!speedTestGroupName || !proxyView) return null
    if (proxyView.global?.name === speedTestGroupName) return proxyView.global
    return (
      proxyView.groups.find((group) => group.name === speedTestGroupName) ??
      null
    )
  }, [proxyView, speedTestGroupName])

  /**
   * 打开指定代理组的测速弹窗。
   */
  const handleOpenSpeedTest = useCallback((group: IRenderItem['group']) => {
